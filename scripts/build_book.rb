...
PARTS = [
  ...
  ["III", "Evidence, integrity, and the lab", "Sources, determinism, parameters, costs, initial conditions, research charter, R0 protocol, and the open-source adoption register.",
   (13..20).map { |n| [Pathname.new(Dir[ROOT.join(format("wiki/%02d-*.md", n))].first).relative_path_from(ROOT).to_s, "Research #{format('%02d', n)}", "Research and hardening"] }],
